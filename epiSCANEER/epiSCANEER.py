...
from . import msa, coe, iscalc
import numpy as np
import networkx as nx
import csv
from collections import Counter

# Cache of amino-acid pair counts per residue pair of the current gene.
# It only memoises what get_num()/get_frequency() would recount for every
# candidate mutation, so results are unchanged.
_PAIR_COUNT_CACHE = {}
...
def _pair_counts(msa_dic, pos1, pos2):
    """(Counter of gap-free AA pairs, number of such pairs) at (pos1, pos2)."""
    key = (pos1, pos2)
    cached = _PAIR_COUNT_CACHE.get(key)
    if cached is None:
        counts = Counter()
        total = 0
        for seq in msa_dic.values():
            pair = seq[pos1-1] + seq[pos2-1]
            if "-" in pair:
                continue
            counts[pair] += 1
            total += 1
        cached = (counts, total)
        _PAIR_COUNT_CACHE[key] = cached
    return cached


def get_frequency(msa_dic, pos1, pos2, AA1, AA2):
    counts, total = _pair_counts(msa_dic, pos1, pos2)
    return counts[AA1 + AA2] / float(total)


def get_num(msa_dic, pos1, pos2, AA1, AA2):
    counts, _total = _pair_counts(msa_dic, pos1, pos2)
    return counts[AA1 + AA2]
...
def get_multiSCI(msa_dic, seq, G, pos1, pos2, AA1, AA2):
    ...
    # The remaining edges of the network contribute 0 to the mean, so the
    # mean is the sum of the affected edges over the total number of edges.
    return sum(SCI_list) / float(G.number_of_edges())
...
def final_epiSCI_normalization(base_pth, prefix):
    ...
    if not rows:
        raise ValueError("no mutation scored in %s" % infile)

    sci_vals = [r["MUT_SCI"] for r in rows]
    sci_min = min(sci_vals)
    sci_max = max(sci_vals)
    sci_range = sci_max - sci_min

    for r in rows:
        r["SCI_norm"] = (r["MUT_SCI"] - sci_min) / sci_range if sci_range else 0.0
    ...
    if not final_rows:
        raise ValueError("no double mutation found in %s" % infile)
...
def calc_epiSCI(base_pth, msa_dic, pm, prefix):
	...
	with open(out_path, "w") as fo:
		...
		for num, (res1, res2) in enumerate(edges):
			...
			# sorted() keeps the output order - and therefore the tie-breaking
			# in final_epiSCI_normalization() - reproducible across runs
			for AA1 in sorted(AA_set1):
				for AA2 in sorted(AA_set2):
	...
	_PAIR_COUNT_CACHE.clear()
	final_epiSCI_normalization(base_pth, prefix)
